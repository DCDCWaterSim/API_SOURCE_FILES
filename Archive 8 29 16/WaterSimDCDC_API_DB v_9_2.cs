 ...
 * Version 5.2
 * Keeper Quay
 *   Requires at least WaterSimDCDC_API_ver_4.cs, WaterSimDCDC_Utilities v 4.cs
 * ******************************************/
        ...
        internal class FastAppendColumnInfoList
        {
            ...
            public bool AppendAnnualData(string DataTableName, string ScnName, SimulationResults AnnualResults)
            {
                switch (FSQLServer)
                {
                    case SQLServer.stAccess:
                        {
                            if (dbFastAppendAnnualData(DataTableName, ScnName, AnnualResults)) // DbRun_AnnualResults.GetAllYears()))
                            //if (dbFastestAppendAnnualData(DataTableName, ScnName, AnnualResults))//DbRun_AnnualResults))// Very fast only works with Microsoft ACE driver 
                            {
                            }
                            else
                            {
