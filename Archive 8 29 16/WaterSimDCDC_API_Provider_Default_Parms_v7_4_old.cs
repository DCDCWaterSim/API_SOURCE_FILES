...
namespace WaterSimDCDC
{
     public partial class WaterSimManager
    {
         ...
         ProviderIntArray defaultGPCD = new ProviderIntArray(0);
         //
         const string FileBuildDefault = "07.30.15_11:21:00";
         //
     partial void initialize_Provider_Default_ModelParameters()   
        {
         ...
         //
            int[] min_alter_GPCD = new int[ProviderClass.NumberOfProviders] {
                    ...
                    50    , //0 ad :       Adaman_Mutual, (the remaining data come from the excell data sheet for the model)
                    50   , //1 wt :        White_Tanks, - used average from all data
                    50    , //2 pv :        Paradise_Valley,
                    50   , //3 su :        Sun_City,
                    50   , //4 sw :        Sun_City_West,
                    50   , //5 av :        Avondale,
                    50   , //6 be :        Berneil,
                    50   , //7 bu :        Buckeye,
                    50   , //8 cf :        Carefree,
                    50   , //9 cc :        Cave_Creek,
                    50   , //10 ch :        Chandler,
                    50   , //11 cp :        Chaparral_City,
                    50   , //12 sp :        Surprise,
                    50   , //13 cu :        Clearwater_Utiliti
                    50   , //14 dh :        Desert_Hills,
                    50   , //15 em :        El_Mirage,
                    50   , //16 gi :        Gilbert,
                    50   , //17 gl :        Glendale,
                    50   , //18 go :        Goodyear,
                    50   , //19 lp :        Litchfield_Park,
                    50   , //20 me :        Mesa,
                  // 50	 , //21 no :       No provider
                  // 50	 , //22 op :       Other Provider
                    50   , //23 pe :        Peoria,
                    50   , //24 ph :        Phoenix, - used data from a 4/23/12 document- GPCD city of phoenix 1990 to 2011
                    50   , //25 qk :        Queen_Creek,
                    50   , //26 rg :        Rigby,
                    50   , //27 rv :        Rio_Verde,
                    50   , //28 ry :        Rose_Valley,
                    50   , //29 sc :        Scottsdale,
                    50   , //30 sr :        Sunrise,
                    50   , //31 te :        Tempe,
                    50   , //32 to :        Tolleson,
                    50   , //33 vu :        Valley_Utilities,
                    50    //34 we :        West_End,   
                };
            ...
            double[] pct_alter_GPCD = new double[ProviderClass.NumberOfProviders] {
                     ...
                     2.76,    //0 ad :        Adaman_Mutual, (the remaining data come from the excell data sheet for the model)
                     0,    //1 wt :         White_Tanks, - used average from all data
                     0,    //2 pv :        Paradise_Valley, 1
                    -5.536,    //3 su :         Sun_City,
              ...
              switch (up)
              {
                  ...
                  case(-1):
                         for (int j = 2000; j < 2086; j++)
                        {
                            yr = Convert.ToDouble(j);
                            gpcd = pct_alter_GPCD[i]*yr + intercept_GPCD[i];
                            gpcd=  Math.Max(minGPCD_pct,gpcd);
                            response_GPCD[i] = 0;
                            if(j == 2013)
                            {
                                start_gpcd[i]=Convert.ToInt32(gpcd);
                            ...
                            if (2084 < j)
                            {
                                ...
                                {
                                    defaultTargetGPCD[i] = min_alter_GPCD[i];
                                }
                        ...
                        response_GPCD[i] = Convert.ToInt32(Math.Max(minGPCD_pct, alter_PCT));
                      break;
                  default:
                      break;
              }
          ...
          temp.Values = defaultGPCD.Values = response_GPCD;//pct_alter_GPCD;
          //
