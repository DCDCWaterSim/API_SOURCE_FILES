...
namespace WaterSimDCDC
{
    public partial class WaterSimManager
    {
        ...
        const string FileBuild = "06.13.14_14:16:00";
        ...
        partial void initialize_WebInterface_DerivedParameters()
        {
            ...
            this.ParamManager.AddParameter(new ModelParameterClass(eModelParam.epEnvironmentalFlow_AFa, "Environmental Flows", "ENFLOAMT", rangeChecktype.rctCheckRange, 0, 500000, geti_FlowEnvironment_AF, seti_FlowEnvironment_AF, RangeCheck.NoSpecialBase));
        ...
        {
            Web_AgricultureTransferToMuni_PCT = 50;
            ...
            if (!FModelLocked)
            {
                ...
                for (int p = 0; p < MaxPump.Length; p++)
                {
                    ...
                    if (0 < _ws.get_WaterFromAgPumpingMax[p])
                    {
                        ...
                        // DAS 03.06.14
                      //  _ws.WaterFromAgPumping[p] = one[p];
                    }
                    //
                    if (0 < _ws.get_WaterFromAgSurfaceMax[p])
                    {
                        ...
                        // DAS 03.06.14
                      //  _ws.WaterFromAgSurface[p] = two[p];
                    }
                }
                //
                One.Values = one;
                ...
                // Uncomfortable with this code. 03.15.14 DAS
              //  WaterAugmentation.setvalues(Two);
            }
        ...
        //public void WaterForEnvironment_AF(int AF)
        //{
        //    double toCO = 0.8;
        //    double toSV = 0.2;
        //    _ws.set_FlowToEnvironment_CO = Convert.ToInt32( AF*toCO);
        //    _ws.set_FlowToEnvironment_Salt = Convert.ToInt32(AF*toSV*0.7);
        //    _ws.set_FlowToEnvironment_Verde = Convert.ToInt32(AF*toSV*0.3);
        //}
        /// <summary>
        /// Acre-feet of water for environment
        /// </summary>
        /// <param name="Total"></param>
        public void WaterForEnvironment_AF(int Total)
        {
            // Units Acre-feet per annum-1
            double value = 0;
            //
            double totalFlow = _ws.get_ColoradoRiverFlow +
                _ws.get_SaltTontoAnnualFlow + _ws.get_VerdeAnnualFlow;
            double ratioCO = 0;
            ratioCO = _ws.get_ColoradoRiverFlow / totalFlow;
            //
            double ratioSalt = 0;
            ratioSalt = _ws.get_SaltTontoAnnualFlow / totalFlow;
            double CO_1 = 0;
            double ST_1 = 0;
            double V_1 = 0;
            //
        ...
        #region Personal
            ...
            internal double[] TempOutdoor = new double[ProviderClass.NumberOfProviders];
            ...
            {
                //Personal_DemandOption3(value);
                Personal_DemandOption4(value);
                //
                // ADDED QUAY 3/9/2014
                PCT_Personal = value;
                //--------------
            }
            private void Personal_DemandOption3(int Switch)
            {
                //
                ...
                switch (Switch)
                {
                    ...
                    default:
                        ...
                        //                
                        int reduceGPCD_pct = 1;
                        if (30 <= Switch)
                        {
                            reduceGPCD_pct = 100 - Switch;
                        }
                        //
                        Provider_Demand_Option = 3;
                        //
                        ...
                        for (int p = 0; p < ProviderClass.NumberOfProviders; p++)
                        {
                            TempOutdoor[p] = Convert.ToDouble(OutDoor[p]) * addOutdoor;
                            RESOut[p] = Math.Max(minOut, Math.Min(maxOut, Convert.ToInt32(TempOutdoor[p])));
                        }
                        PCT_Reduce_GPCD = reduceGPCD_pct;
                        //
                        Temp.Values = RESOut;
            ...
            public void Personal_DemandOption4(int Switch)
            {
                int year = _CurrentYear;
                ...
                switch (Switch)
                {
                    ...
                    default:
                        ...
                        if (year <= 2012) Provider_Demand_Option = 3;
                        //
                        OutDoor = PCT_Outdoor_WaterUseRes.getvalues().Values;
                        ...
                        for (int p = 0; p < ProviderClass.NumberOfProviders; p++)
                        {
                            ...
                            else
                            {
                                ...
                                if (alter[p] == 0)
                                {
                                    GPCDt[p] = GPCDbuffer[p] - subt;
                                }
                                // Increasing trends
                                else if (0 < alter[p])
                                {
                                    GPCDt[p] = GPCDbuffer[p] - subt;
                                    addOutdoor = modOutdoor + 0.1;
                                }
                                ///Decreasing Trend
                                else
                                {
                            ...
                            GPCDuse[p] = Math.Max(minGPCD, GPCDt[p]);
            ...
            private int Class(int Switch)
            {
                ...
                {
                    case 1:
                        gpcd = Convert.ToInt32(Convert.ToDouble(1) * addGPCD);
                        break;
                    case 2:
                        gpcd = Convert.ToInt32(Convert.ToDouble(2) * addGPCD);

                        break;
                    case 3:
                        gpcd = Convert.ToInt32(Convert.ToDouble(3) * addGPCD);

                        break;
                    case 4:
                        gpcd = Convert.ToInt32(Convert.ToDouble(4) * addGPCD);

                        break;
                    case 5:
                        gpcd = Convert.ToInt32(Convert.ToDouble(5) * addGPCD);

                        break;
                    case 6:
                        gpcd = Convert.ToInt32(Convert.ToDouble(7) * addGPCD);

                        break;
                    case 7:
                        gpcd = Convert.ToInt32(Convert.ToDouble(9) * addGPCD);

                        break;
                    case 8:
                        gpcd = Convert.ToInt32(Convert.ToDouble(11) * addGPCD);

                        break;
                    case 9:
                        gpcd = Convert.ToInt32(Convert.ToDouble(13) * addGPCD);

                        break;
                    case 10:
                        gpcd = Convert.ToInt32(Convert.ToDouble(15) * addGPCD);

                        break;
                    case 11:
                        gpcd = Convert.ToInt32(Convert.ToDouble(17) * addGPCD);
